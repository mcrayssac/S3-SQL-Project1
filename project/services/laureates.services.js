...
const queries = require("../queries/laureates.queries");
...
const getAllLaureates = (id, callback) => {
    if (id !== null){
        pool.query(queries.list_laureate, [id],(error, results) => {
            if(error) return callback(error);
            else {
                let tabResults = {};
                tabResults.prenom = results.rows[0].prenom;
                tabResults.nom = results.rows[0].nom;
                tabResults.prixRemportes = [];
                for (item of results.rows) tabResults.prixRemportes.push({annee: item.annee, categorie: item.categorie, motivation: item.motivation});
                return callback(null, tabResults);
            }
        });
    } else {
        pool.query(queries.list_laureates, (error, results) => {
            if(error) return callback(error);
 ...
 */
const deleteLaureates = (id, callback) => {
    id = parseInt(id);
    pool.query(queries.laureate, [id], (error, results) => {
        if(error) return callback(error);
        else if (results.rowCount === 0) {
            return callback("Id laureate not exits");
        } else {
            pool.query(queries.deleteLaureates, [id], (error, results) => {
                if(error) return callback(error);
                else return callback(null, "Delete successfully");
            })
        }
    });
...
const putLaureates = (id, year, category, motivation, callback) => {
    id = parseInt(id);
    year = parseInt(year);
    pool.query(queries.selectMotivation, [id, year, category], async (error, results) => {
        if(error) return callback(error);
        else if (results.rowCount === 0) {
            return callback("Laureate prize not exits");
        } else {
            let row_prize = await getPrize(year, category);
            pool.query(queries.updateMotivation, [motivation, id, row_prize], (error, results) => {
                if(error) return callback(error);
...
async function getPrize(year, category){
    let res = await pool.query(queries.selectPrize, [year, category]);
    return res.rows[0].id_prize;
}
