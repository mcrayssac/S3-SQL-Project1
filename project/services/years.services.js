...
const queries = require("../queries/years.queries");
...
const getLaureatesYearWinners = (callback) => {
    pool.query(queries.getLaureatesYearWinners, (error, results) => {
...
const getYearsPrizesEmpty = (callback) => {
    pool.query(queries.getYearsPrizesEmpty, (error, results) => {
        if(error) return callback(error);
        else return callback(null, results.rows)
    });
...
const getYearsPrizesSort = (sort, callback) => {
    if (sort.charAt(0) === "d"){
        pool.query(queries.getYearsPrizesSortDesc, (error, results) => {
            if(error) return callback(error);
            else return callback(null, results.rows)
        });
    } else {
        pool.query(queries.getYearsPrizesSortAsc, (error, results) => {
            if(error) return callback(error);
            else return callback(null, results.rows)
        });
    }
