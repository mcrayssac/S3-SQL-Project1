...
CREATE TABLE PRIZE(
   ...
   CONSTRAINT fk_prize_category FOREIGN KEY(id_category) REFERENCES CATEGORY(id_category) ON DELETE CASCADE
);
...
CREATE TABLE Recoit(
   ...
   CONSTRAINT fk_recoit_id_laureate FOREIGN KEY(id_laureate) REFERENCES LAUREATE(id_laureate) ON DELETE CASCADE,
   CONSTRAINT fk_recoit_id_prize FOREIGN KEY(id_prize) REFERENCES PRIZE(id_prize) ON DELETE CASCADE
);
